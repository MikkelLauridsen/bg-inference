...
reduceTypeConstraints tcs = aux (tcs, Set.empty)
    where
        ...
        reduceTypeConstraint (TCSConditionalSubsumption cgammas env (TChannel sigma ix ts) (TChannel sigma' jx ss)) (tcs', ucs') = (tcs' `Set.union` tcs'', ucs' `Set.union` ucs'')
            where
                tcs'' = Set.fromList ( [TCSConditionalSubsumption (UCCSSubset sigma (UCSet (Set.singleton UCIn)) : cgammas) env t s | (t, s) <- Prelude.zip ts ss] ++
        ...
        reduceTypeConstraint (TCSConditionalSubsumption cgammas (vphi, phi) (TServ ix is sigma kx ts) (TServ jx _ sigma' kx' ss)) (tcs', ucs') = (tcs' `Set.union` tcs'', ucs' `Set.union` ucs'')
            where
                tcs'' = Set.fromList ( [TCSConditionalSubsumption (UCCSSubset sigma (UCSet (Set.singleton UCIn)) : cgammas) (vphi `Set.union` is, phi) t s | (t, s) <- Prelude.zip ts ss] ++
...
satisfiesUC f (USCConditional cgammas cgamma) = satisfiesAll f cgammas || satisfies f cgamma
